...
%Author: JP
%Version: 1.01 
% Default analysis work up for Hbond/H0contact list produced in VMD
%The file format expected is tab delm. for 
%       Hbond:   Frame #Hb #H20b following pattern[3let ResNum]
%       H0bond:  Frame #Hb following pattern[3let ResNum]
%ResNum comes from the file put into NAMD, so numbering specific
%%
% Notes: 1-05-2020
%   Need to add offset number for GA3 and GID1A
%%%
...
tic;
offset=1;
GAname="GID1A-GA7_Alt";
% h0bondUncapMapper(GAname,offset);
h0bondEgressionMapper(GAname,offset);
hbondEgressionMapper(GAname,offset);
% hbondUncapMapper(GAname,offset);
toc;

function hbondMapper(sysName, offset)
finputData=["out-MoRF1_Hbond_3.5.dat","out-MoRF2_Hbond_3.5.dat",...
    "out-MoRF3_Hbond_3.5.dat","out-lig_Hbond_3.5.dat",...
    "out-ligWater_Hbond_3.5.dat"];
...
for a=1: length(finputData)
if( isfile(strcat("./Hao_output/",finputData{a})) )
   fid = fopen("./Hao_output/"+finputData{a});
...
% ALternate way where the whole array is in one cell.
  ...
  Names{lineCounter} = {tempLine{4:length(tempLine)-1}};
% Read the next line.
  textLine = fgets(fid);
...
figure(1);
hold on;
plot([ca{:,1}],[ca{:,2}],'--ro','LineWidth',2,...
                     'MarkerEdgeColor','k',...
                     'MarkerFaceColor','g',...
                     'MarkerSize',5)
plot([ca{:,1}],[ca{:,3}],'--bo','LineWidth',2,...
                     'MarkerEdgeColor','k',...
                     'MarkerFaceColor','g',...
                     'MarkerSize',5)
hold off;
savefig(figure(1),"HbondFreq"+uniqueTag{a});                 
%%
% Make the propensity scale onto pdb structure
%plot Histogram of H-bond contact residueID
ResContactList = containers.Map;
ResidueCountCorrection = 110; %DELLA length
%The NAMD file takes all residiues for counting; 
NumOfRes = 0;
for i=1 : length(Names)
    fold = length(Names{i})/2;
    for j=1 : length(Names{i})/2 %half array is 3 letter, other res number
        modNum=num2str( str2num(Names{i}{j+fold})+offset);
        ResidueID2 = strcat(Names{i}{j},modNum);
%         ResidueID2 = strcat(Names{i}{j},Names{i}{j+fold});
        %ResNum=str2double(Names{i}{j+fold})-ResidueCountCorrection;
        %ResidueID = strcat(Names{i}{j},num2str(ResNum));
        if ~ isKey(ResContactList,ResidueID2)
            NumOfRes = NumOfRes+1;
            ResContactList(ResidueID2) = 1;
        else
            temp = ResContactList(ResidueID2);
            temp = temp + 1;
            ResContactList(ResidueID2) = temp;
        end
    end
end
% basic histogram of h-bond residues
figure(2);
bar( cell2mat( values(ResContactList) ) );
set(gca,'XTick',[1:length(keys(ResContactList))]);
xtickangle(90);
set(gca,'xticklabel', keys(ResContactList));
savefig(figure(2),"HbondHist"+uniqueTag{a});
hold off;
save("HbondTable"+uniqueTag{a}, 'ResContactList');

% % ------------------- Plot colmn mean in FlxFlx onto PDB file
PDBdata = pdb2mat(finputStruct);
normContactFreq=arrayfun(@(x) x{:}/length(Names),ResContactList.values);
normResContactList = containers.Map(ResContactList.keys,normContactFreq);
...
for i=2:length((PDBdata.resNum))
    tempResID=strcat(PDBdata.resName{i},num2str(tempResNum(i)-offset));
...
PDBdata.outfile = "ref_contacts"+uniqueTag{a}+".pdb";
mat2pdb(PDBdata);
%--------------------------------------------------------------------------
%%
% plot of h bond residues over time (attendence over time?)
figure(3);
ylim([0, length(ResContactList)]);
%ax.YTick = [0, length(ResContactList)];
%ax.YTickLabels = ResContactList.keys;
logicIndex = ResContactList.keys';
...
%         ResidueID2 = strcat(Names{i}{j},Names{i}{j+fold});
    ...
    plot(x, present', 'b*', 'MarkerSize', 10, 'LineWidth', 3);
    
end
savefig(figure(3),"HbondAttendence"+uniqueTag{a});
hold off;
%%
pause(20);
close all
clearvars -except finputData finputStruct uniqueTag sysName offset

end
end
function hbondEgressionMapper(sysName, offset)
finputData=["lig_Hbond_3.5.dat",...
    "ligWater_Hbond_3.5.dat"];
finputStruct="ref.pdb";
uniqueTag=[strcat(sysName,"_lig"),strcat(sysName,"_ligWater")];

for a=1: length(finputData)
hold on;
pdbFrame = pdb2mat(finputStruct);
FileList = dir(strcat('./Alt_output/egression*',finputData{a}));
colourList=hsv(length(FileList));
TotalResList={};
TotalWaterList={};
for i=1 : length(FileList)
legnendList(i)="Egression "+i;
if( isfile(strcat('./Alt_output/',FileList(i).name)) )
   fid = fopen(strcat('./Alt_output/',FileList(i).name));
   textLine = fgets(fid); % Read first line.
   lineCounter = 1;
...
for k=1 : length(FileList)
    ...
    for i=1 : length(Names)
        ...
        for j=1 : length(Names{k,i})/2 %half array is 3 letter, other res number
            modNum=num2str( str2num(Names{k,i}{j+fold})+offset);
            ResidueID2 = strcat(Names{k,i}{j},modNum);
...
function hbondUncapMapper(sysName, offset)
...
hold on;
pdbFrame = pdb2mat(finputStruct);
FileList = dir(strcat('./Hao_output/uncap*',finputData{a}));
colourList=hsv(length(FileList));
TotalResList={};
TotalWaterList={};
for i=1 : length(FileList)
legnendList(i)="Uncap "+i;
if( isfile(strcat('./Hao_output/',FileList(i).name)) )
   fid = fopen(strcat('./Hao_output/',FileList(i).name));
   textLine = fgets(fid); % Read first line.
   lineCounter = 1;
...
hold off;
legend();
savefig(figure(1),"uncapHbondFreq"+uniqueTag{a});  
...
legend(["residue","water"]);
hold off;
savefig(figure(2),"uncapAvgHbondFreq"+uniqueTag{a});
%%
% Make the propensity scale onto pdb structure
%plot Histogram of H-bond contact residueID
ResContactNameList={};%containers.Map; cant use, fucking sorts the keys
...
savefig(figure(3),"HbondHistUncap"+uniqueTag{a});
save("HbondTableUncap"+uniqueTag{a}, 'ResContactNameList', 'ResContactNumList');
...
%disp(tempBFactor);
% PDBdata.betaFactor = tempFlxFlxIndx;
PDBdata.outfile = "ref_Hcontacts_uncap"+uniqueTag{a}+".pdb";
mat2pdb(PDBdata);
%--------------------------------------------------------------------------
%%
% plot of h bond residues over time (attendence over time?)
...
savefig(figure(4),"HbondAttendenceUncap"+uniqueTag{a});
hold off;
%Plot Fourier transformation of Attendence
%Note that currently this is a all sum attendence. Not egression by
%egression like it probably should be
...
function h0bondMapper(sysName, offset)
finputData=["out-MoRF1_H0bond_3.5.dat",...
    "out-MoRF2_H0bond_3.5.dat","out-MoRF3_H0bond_3.5.dat",...
    "out-lig_H0bond_3.5.dat"];
finputStruct="ref.pdb";
uniqueTag=[strcat(sysName,"_MoRF1"),strcat(sysName,"_MoRF2"),...
strcat(sysName,"_MoRF3"),strcat(sysName,"_lig")];
...
% ALternate way where the whole array is in one cell.
  ca2{lineCounter} = numbers;
  tempLine = split(textLine)';
  Names{lineCounter} = {tempLine{3:length(tempLine)-1}};
% Read the next line.
  textLine = fgets(fid);
  lineCounter = lineCounter + 1;
end
...
plot([ca{:,1}],[ca{:,2}],':o','LineWidth',2,...
                     'Color','b',...
                     'MarkerEdgeColor','k',...
                     'MarkerFaceColor','g',...
                     'DisplayName',uniqueTag{a},...
                     'MarkerSize',3)

hold off;
legend();
savefig(figure(1),"egressH0bondFreq"+uniqueTag{a});  
...
% errorbar(x,AvgResFreq, StdResFreq)
% hold off;
% savefig(figure(2),"AvgH0bondFreq"+uniqueTag);
%%
% Make the propensity scale onto pdb structure
%plot Histogram of H-bond contact residueID
ResContactNameList={};%containers.Map; cant use, fucking sorts the keys
...
savefig(figure(3),"H0bondHist"+uniqueTag{a});
save("H0bondTable"+uniqueTag{a}, 'ResContactNameList','ResContactNumList');
...
%disp(tempBFactor);
% PDBdata.betaFactor = tempFlxFlxIndx;
PDBdata.outfile = "ref_H0contacts_"+uniqueTag{a}+".pdb";
mat2pdb(PDBdata);
%--------------------------------------------------------------------------
%%
% plot of h bond residues over time (attendence over time?)
...
for i=1:1:length(Names)
    ...
    x = i * ones(1, length(present));
    plot(x, present', '*','Color',colList(k,:),'MarkerSize', 2, 'LineWidth', 3);
    logicMatrix(:,i)= logical(logicMatrix(:,i))+logical(logicArray);
end

savefig(figure(4),"H0bondAttendence"+uniqueTag{a});
hold off;
%%
pause(20);
close all
...
function h0bondUncapMapper(sysName, offset)
finputData=["lig_H0bond_3.5.dat"];
finputStruct="ref.pdb";
uniqueTag=[strcat(sysName,"_lig")];
...
% ALternate way where the whole array is in one cell.
  ...
  TotalResList{i,lineCounter} = ca{lineCounter,2};
  tempLine = split(textLine)';
  Names{i,lineCounter} = {tempLine{3:length(tempLine)-1}};
% Read the next line.
  textLine = fgets(fid);
  lineCounter = lineCounter + 1;
end
...
hold off;
legend();
savefig(figure(1),"uncapH0bondFreq"+uniqueTag{a});  
...
errorbar(x,AvgResFreq, StdResFreq)
hold off;
savefig(figure(2),"uncapAvgH0bondFreq"+uniqueTag{a});
%%
% Make the propensity scale onto pdb structure
%plot Histogram of H-bond contact residueID
ResContactNameList={};%containers.Map; cant use, fucking sorts the keys
...
savefig(figure(3),"H0bondHistUncap"+uniqueTag{a});
save("H0bondTableUncap"+uniqueTag{a}, 'ResContactNameList','ResContactNumList');
...
%disp(tempBFactor);
% PDBdata.betaFactor = tempFlxFlxIndx;
PDBdata.outfile = "ref_H0contacts_Uncap"+uniqueTag{a}+".pdb";
mat2pdb(PDBdata);
%--------------------------------------------------------------------------
%%
% plot of h bond residues over time (attendence over time?)
...
for k=1 : length(FileList)
    for i=1:1:length(Names)
        ...
        x = i * ones(1, length(present));
        plot(x, present', '*','Color',colList(k,:),'MarkerSize', 12-k, 'LineWidth', 3);
        logicMatrix(:,i)= logical(logicMatrix(:,i))+logical(logicArray);
    end
end
savefig(figure(4),"H0bondAttendence"+uniqueTag{a});
...
function h0bondEgressionMapper(sysName, offset)
finputData=["lig_H0bond_3.5.dat"];
finputStruct="ref.pdb";
uniqueTag=[strcat(sysName,"_lig")];
...
hold on;
pdbFrame = pdb2mat(finputStruct);
% FileList = dir(strcat('./VMD_output/',finputData,'*_H0bond_3.5.dat'));
FileList = dir(strcat('./Alt_output/egression*',finputData{a}));
colourList=hsv(length(FileList));
TotalResList={};
for i=1 : length(FileList)
...
errorbar(x,AvgResFreq, StdResFreq)
hold off;
savefig(figure(2),"egressAvgH0bondFreq"+uniqueTag{a});
%%
% Make the propensity scale onto pdb structure
%plot Histogram of H-bond contact residueID
ResContactNameList={};%containers.Map; cant use, fucking sorts the keys
...
savefig(figure(3),"H0bondHistEgress"+uniqueTag{a});
save("H0bondTableEgress"+uniqueTag{a}, 'ResContactNameList','ResContactNumList');
...
%disp(tempBFactor);
% PDBdata.betaFactor = tempFlxFlxIndx;
PDBdata.outfile = "ref_H0contacts_egress"+uniqueTag{a}+".pdb";
mat2pdb(PDBdata);
%--------------------------------------------------------------------------
%%
% plot of h bond residues over time (attendence over time?)
...
savefig(figure(4),"H0bondAttendenceEgress"+uniqueTag{a});
hold off;
